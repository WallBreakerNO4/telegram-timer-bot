import TelegramBot, { TelegramExecutionContext, type TelegramUpdate } from '@codebam/cf-workers-telegram-bot';
...
const TZM_SYSTEM_PROMPT = [
  '你是一个时间解析器。你的任务是把用户的中文自然语言时间表达转换为一个单次时间点。',
  '输入会以 JSON 字符串形式提供：{ expression, requesterTimezone, currentTime }。',
  '请以 requesterTimezone 作为用户语境时区来解析相对时间（例如 明天/下周），currentTime 是当前时间（ISO 8601，UTC）。',
  '输出必须严格符合提供的 JSON Schema：',
  '- ok: boolean（能解析为单次时间点则 true）',
  '- isoTimestamp: string（RFC3339/ISO8601，必须包含时区偏移，例如 2026-02-10T17:00:00+08:00）',
  "- confidence: 'high' | 'medium' | 'low'",
  '- assumptions: string[]（若需要补全日期/时间/年份等，说明采用的假设）',
...
function buildTzmMessage(header: string, lines: string[]): string {
...
function isPeriodicExpression(expression: string): boolean {
...
function parseTzmAiResponse(aiResult: unknown): ParseResult<TzmParseResult> {
...
function parseArgumentsFromContext(ctx: TelegramExecutionContext): string[] {
...
function determineCommandFromContext(bot: TelegramBot, ctx: TelegramExecutionContext, args: string[], env: Env): string {
  // 先处理特殊 update 类型
  switch (ctx.update_type) {
    case 'photo':
